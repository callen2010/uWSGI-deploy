...
class base::install {
  ...
  exec { 'hostname change': command => '/bin/echo `cat /etc/hostname`-`date +%D-%T` > /etc/hostname; /bin/echo \'127.0.0.1 localhost localhost.localdomain\' `cat /etc/hostname` > /etc/hosts; hostname `cat /etc/hostname`;', require => Exec['timezone set'], onlyif => '/bin/grep "-" /etc/hosts; [ $? -eq 1 ]',}  
